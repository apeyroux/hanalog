...
main = do 
	args <- getArgs
	ln <- readFile (args!!0)
	mapM_ (\(hit, user) -> putStrLn $ "IP : " ++ user ++ "\tNb action : " ++ show hit) $ reverse . sort . map (\r -> (length r, head r)) $ group . sort . map (\l -> case parseLine l of Right res -> getIP res)$ lines ln
